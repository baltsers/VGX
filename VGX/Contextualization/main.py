...
from dataset import GraphAttenDataset, TextVocab, UniTextVocab, collect_fn, CTTextVocab
...
def train():
    ...
    # dataset
    parser.add_argument("--on_memory", type=boolean_string, default=False, help="Loading datasets into memory")

    # dataset size
    parser.add_argument("--max_code_length", type=int, default=512, help="")
    parser.add_argument("--max_target_len", type=int, default=256, help=' <eos or sos> + true len of method name')
    ...
    parser.add_argument("--batch_size", type=int, default=24, help="number of batch_size")
    parser.add_argument("--accu_batch_size", type=int, default=48,
                        help="number of real batch_size per step, save gpu memory")
    parser.add_argument("--val_batch_size", type=int, default=48, help="number of batch_size of valid")
    ...
    # graph embedding
    parser.add_argument("--graph_embedding_size", type=int, default=128, help="embedding size of subgraph")
    parser.add_argument("--max_abs_subgraph_num", type=int, default=128,
                        help="Max absolute subgraph number in a sample")
    
    parser.add_argument("--max_subgraph_num", type=int, default=256,
                        help="Max relative subgraph number in a sample")
    # transformer
    ...
    # Graph encoding
    parser.add_argument("--relation_graph", type=boolean_string, default=True, help="Whether to use relative graph")
    parser.add_argument("--absolute_graph", type=boolean_string, default=True, help="Whether to use absolute graph")
    parser.add_argument("--graph_value", type=boolean_string, default=True,
                        help="Whether to use the weight sum of Value of relative graph")
    parser.add_argument("--ap_kq", type=boolean_string, default=True,
                        help="The projection of Key and Query for absolute graph encoding")
    parser.add_argument("--rp_kv", type=boolean_string, default=True,
                        help="The projection of Key and Value for relative graph encoding")
    ...
    parser.add_argument("--test", type=boolean_string, default=False, help="Whether to test")
    parser.add_argument("--load_checkpoint", type=boolean_string, default=True,
                        help="load checkpoint for continue train or infer")
    parser.add_argument("--checkpoint", type=str, default='pretrained_all2.pth', help="the checkpoint file path")
    ...
    if args.data_debug:
        train_dataset = GraphAttenDataset(args, s_vocab, t_vocab, type_='valid')
    else:
        train_dataset = GraphAttenDataset(args, s_vocab, t_vocab, type_='train')
        if has_binary:
            train_dataset2 = GraphAttenDataset(args, s_vocab, t_vocab, type_='binary_train')
    print("Loading Valid Dataset")
    valid_dataset = GraphAttenDataset(args, s_vocab, t_vocab, type_='valid') 
    if has_binary:
        valid_dataset2 = GraphAttenDataset(args, s_vocab, t_vocab, type_='binary_test')
    print("Loading Test Dataset")
    test_dataset = GraphAttenDataset(args, s_vocab, t_vocab, type_='test')
    if has_binary:
        test_dataset2 = GraphAttenDataset(args, s_vocab, t_vocab, type_='binary_test')
    if args.on_memory:
    ...
    if args.train:
        ...
        if has_binary:
            train_data_loader2 = DataLoader(train_dataset2, batch_size=args.batch_size, num_workers=num_workers,
                                       shuffle=args.shuffle, collate_fn=collect_fn)
    else:
    ...
    if has_binary:
        valid_data_loader2 = DataLoader(valid_dataset2, batch_size=args.val_batch_size, num_workers=num_workers,
                                   collate_fn=collect_fn)
    valid_infer_data_loader = DataLoader(valid_dataset, batch_size=args.infer_batch_size, num_workers=num_workers,
                                         collate_fn=collect_fn)
    if has_binary:
        valid_infer_data_loader2 = DataLoader(valid_dataset2, batch_size=args.val_batch_size, num_workers=num_workers,
                                   collate_fn=collect_fn)
    test_infer_data_loader = DataLoader(test_dataset, batch_size=args.infer_batch_size, num_workers=num_workers,
                                        collate_fn=collect_fn)
    if has_binary:
        test_infer_data_loader2 = DataLoader(test_dataset2, batch_size=args.infer_batch_size, num_workers=num_workers,
                                        collate_fn=collect_fn)
    ...
    if has_binary:
        trainer = Trainer(args=args, model=model, train_data=train_data_loader, valid_data=valid_data_loader,
                      valid_infer_data=valid_infer_data_loader, test_infer_data=test_infer_data_loader, t_vocab=t_vocab,
                      train_data2=train_data_loader2, valid_data2=valid_data_loader2,
                      valid_infer_data2=valid_infer_data_loader2, test_infer_data2=test_infer_data_loader2
                      )
    ...
    for epoch in range(args.epochs):
        ...
        #trainer.predict(epoch, test=True, is_binary=True)
    epoch=0
    trainer.predict(epoch, test=True)
