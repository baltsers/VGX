...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    df.to_csv("../data/raw_predictions/CodeT5/VulRepair_raw_preds_translate_final_beam1.csv")
...
def main():
    ...
    if args.do_test:
        ...
        tokenizer.add_tokens(["<S2SV_StartBug>", "<S2SV_EndBug>", "<S2SV_blank>", "<S2SV_ModStart>", "<S2SV_ModEnd>"])
        model.resize_token_embeddings(len(tokenizer))
        model.load_state_dict(torch.load('./saved_models/checkpoint-best-loss/model_translate.bin'))
        model.to(args.device)
        pairs=os.listdir('T5_vulgen_test_translate_final_tokenized')
        dataset = {'name': [], 'source': [], 'target': []}
        for i, pair in enumerate(pairs):
            #if i>30:
            #    break
            ...
            #continue
            try:
                f=open('./T5_vulgen_test_translate_final_tokenized2/'+pair+'/'+pair+'_in.c')
                source=f.read()
                source=source.replace(' ', '<S2SV_blank>')
                source=source.replace('!@#$', ' ')
                f.close()
                f=open('./T5_vulgen_test_translate_final_tokenized2/'+pair+'/'+pair+'_out.c')
                target=f.read()
                target=target.replace(' ', '<S2SV_blank>')
                target=target.replace('!@#$', ' ')
                f.close()
