...
class EditPattern:
    ...
    def changeBeforePattern(self, before_pattern):
        self.__before_tree_pattern = before_pattern

    def changeAfterPattern(self, after_pattern):
        self.__after_tree_pattern = after_pattern
    ...
    def matchAst(self, ast, hole_dict = {}):
        ...
        for node_id in ast_node_dict:
            if self.target_node_id != -1 and node_id != self.target_node_id:
                continue
        ...
        if not "!@#$hole" in pattern_root_node.value:
            if ast_root_node.value == pattern_root_node.value and ast_root_node.type == pattern_root_node.type:
                if self.__childrenMatchHelper(ast_root_node.children, pattern_root_node.children, hole_dict):
                    return True
            elif not pattern_root_node.value == '':
                try:
                    re_obj = re.match(pattern_root_node.value, ast_root_node.value)
                except:
                    return False
                if re_obj != None and re_obj.span()[0] == 0 and re_obj.span()[1] == len(ast_root_node.value):
                    return True
            else:
                return False
        else:
        ...
        if not is_after_pattern:
            if matched_root_id == src_subtree_root_node.getIdInAst():
                ...
                if after_root_node == None:
                    return
                if "!@#$hole" in after_root_node.value:
                    if after_root_node.value in hole_dict:
                        subtrees_to_be_added = hole_dict[after_root_node.value]
                        ...
                        self.__after_tree_pattern.copySubtree(0, tgt_tree, tgt_subtree_parent_node_id, len(tgt_tree.getNodeDict()[tgt_subtree_parent_node_id].children))
                else:
            ...
            if "!@#$hole" in after_subtree_root_node.value:
                if after_subtree_root_node.value in hole_dict:
                    ...
                    tgt_tree.addSingleNode(after_subtree_root_node.type, after_subtree_root_node.value, tgt_subtree_parent_node_id, len(tgt_tree.getNodeDict()[tgt_subtree_parent_node_id].children))
