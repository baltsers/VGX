...
def applyT5Location(pattern_src_trees, name_pred_dict):
    patterns = []
    for pattern_src_tree in pattern_src_trees:
        pattern_prevalence_score = pattern_src_tree[0]
        pattern = pattern_prevalence_score[0]
    ...
    if True:
        if src_tree_name in name_pred_dict:
            T5_pred = name_pred_dict[src_tree_name]
            ...
            for j in range(len(src_tree.getNodeDict())):
                ...
                if fuzz.ratio(subtree_code, T5_pred)>90:
                    subtree_id = j
                    break
...
class HierarchicalCluster:
    ...
    def __clustering(self):
        ...
        for root_type in self.working_sets:
            if root_type != "if_stmt" and root_type != "expr_stmt" and root_type != "decl_stmt" and root_type != "for" and root_type != "do" and root_type != "goto" and root_type != "while" and root_type != "switch" and root_type != "case" and root_type != "if_stmt" and root_type != "label":
                continue
        ...
        for root_type in self.working_sets:
            if root_type != "if_stmt" and root_type != "expr_stmt" and root_type != "decl_stmt" and root_type != "for" and root_type != "do" and root_type != "goto" and root_type != "while" and root_type != "switch" and root_type != "case" and root_type != "if_stmt" and root_type != "label":
                continue
            ps.append(os.popen("pypy3 -u cluster.py "+root_type+" > "+'./outs/'+root_type+".out 2> "+'./outs/'+root_type+".err"))
        for p in ps:
            p.read()
    ...
    def applyPattern(self, src_tree, src_tree_name):
        try:
            ...
            if os.path.exists('prevalence_spec4_mutated.pkl'):
                f = open('prevalence_spec4_mutated.pkl', 'rb')
                obj = pickle.load(f)
                patterns = obj[0]
                self.proned_patterns = patterns
                self.proned = True
                ...
                f = open('vulgen_train_final.pkl', 'rb')
                train_pairs = pickle.load(f)
                f.close()
                patterns = []
                for node in self.hierarchical_nodes: 
                ...
                for iii, pattern in enumerate(patterns[12500:]):
                    #if pattern.getBeforePattern().getRootNode().type.type_name == 'if':
                    score = 0
                    success_train_pairs = []
                    success_subtree_root_ids = []
                    ...
                    for jjj,train_pair in enumerate(train_pairs):
                        ...
                        try:
                            ...
                            signal.alarm(0)
                        if len(new_asts)!=0:
                            spec += len(before_ast)/len(new_asts)
                        else:
                            spec += len(before_ast)
                        for kkk, new_ast in enumerate(new_asts):
                            if new_ast.getCode() == after_ast.getCode():
                ...
                patterns = self.proned_patterns
                f = open('prevalence_spec_12500.pkl', 'wb')
                pickle.dump([patterns, self.pattern_prevalence_scores], f)
                f.close()
                import pdb
                pdb.set_trace()
        ...
        for pattern_prevalence_score in self.pattern_prevalence_scores:
            ...
            lines=pattern.__str__().split('\n')
            for l in lines:
                if 'name:' in l:
                    if not l.split('name:')[-1] == ' ':
                        if not '!@#$hole' in l.split('name:')[-1]:
                            prevalence_score = prevalence_score*10
            ...
            pattern_scores.append([pattern_prevalence_score, prevalence_score])
        ...
        #for pattern_prevalence_score in self.pattern_prevalence_scores[:1]:
        ...
        f=open('Contextualization_raw_preds_final2_beam1.pkl', 'rb')
        T5_output=pickle.load(f)
        f.close()
        name_pred_dict={}
        for i in range(len(T5_output['names'])):
            name_pred_dict[T5_output['names'][i]] = T5_output['raw_predictions'][i]

        patterns, new_asts, subtree_root_ids = applyT5Location(pattern_src_trees, name_pred_dict)
